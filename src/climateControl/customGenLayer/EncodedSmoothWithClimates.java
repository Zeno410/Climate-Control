 ...
 * @author Zeno410
 */
public abstract class EncodedSmoothWithClimates extends GenLayerPack {
    private static int deepOcean;
    private static int mushroomIsland;
    public static Logger logger = new Zeno410Logger("EncodedSmoothWithClimates").logger();

    public EncodedSmoothWithClimates(long par1, GenLayer par3GenLayer){
        super(par1);
    ...
    public int[] getInts(int par1, int par2, int par3, int par4){
        ...
        for (int i2 = 0; i2 < par4; ++i2)
        {
            for (int j2 = 0; j2 < par3; ++j2)
            {
                ...
                int decodedk3 = decoded(k3);
                if (decodedk3 == mushroomIsland) {
                    aint1[j2 + i2 * par3] = k3;
                    continue;
                }
                int decodedk2 = decoded(k2);
                int decodedl2 = decoded(l2);
                int decodedi3 = decoded(i3);
                int decodedj3 = decoded(j3);

                if (decodedk2 == decodedl2 && decodedi3 == decodedj3)
                {
                ...
                {
                    if (decodedk2 == decodedl2){
                        k3 = k2;
                    } else if (decodedi3 == decodedj3){
                        k3 = i3;
                    } else {
    ...
    private final int climateSmoothed(int i1,int j1,int k1,int l1,int k2,int l2,int i3,int j3,int k3, int par1, int par2, int i2, int j2) {
        ...
        int decodedi3 = decoded(i3);
        int decodedj3 = decoded(j3);
        if (decodedk2>0&& decodedl2>0&&decodedk2!=deepOcean&&decodedl2!=deepOcean){
            this.initChunkSeed((long)(j2 + par1), (long)(i2 + par2));
            if (decodedi3>0&& decodedj3>0&&decodedi3!=deepOcean&&decodedj3!=deepOcean) {
